...
(
    id        SERIAL PRIMARY KEY NOT NULL,
    email     TEXT               NOT NULL,
    authority TEXT               NOT NULL, -- “ROLE_ADMIN”, "ROLE_OP"...
    CONSTRAINT fk_customer FOREIGN KEY (email) REFERENCES customers (email) ON DELETE CASCADE -- delete one entry in customer will also caused entry with same email to be deleted.
);
